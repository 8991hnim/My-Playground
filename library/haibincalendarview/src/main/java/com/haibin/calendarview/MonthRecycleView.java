...
public class MonthRecycleView extends RecyclerView {
    ...
    private void init() {
        ...
        setAdapter(mAdapter);
        mStickyDecoration = new StickyDecoration(mContext, position -> {
...
//        setLayoutParams(params);
    ...
    private int dpToPx(Float number){
    ...
    private Float spToPx(Float number){
    ...
    public class StickyDecoration extends ItemDecoration {
        ...
        public void getItemOffsets(Rect outRect, View view, RecyclerView parent, State state) {
            ...
            //Add padding to the first one in the same group
...
//                outRect.left = topHead / 2;
        ...
        public void onDrawOver(Canvas c, RecyclerView parent, State state) {
            ...
            //Get the number of currently visible items, excluding grouping items, pay attention to distinguish the following
            int childCount = parent.getChildCount();
            //Get the number of all items, it is not recommended to use the Adapter to obtain
            int itemCount = state.getItemCount();
            int left = 0;
            int right = parent.getWidth() - dpToPx(10f);
            String preDate;
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                //The next one is not the same as the current one, move the current one
                if (position + 1 < itemCount) {
                    String nextData = callback.getData(position + 1);
                    if (!currentDate.equals(nextData) && viewBottom < textY) {//The last view in the group entered the header
                        textY = viewBottom;
                    }
                }
                Rect rect = new Rect(left, (int) textY - topHead, right, (int) textY);
                c.drawRect(rect, paint);
                //Draw the text baseline, the text is drawn from the bottom of the drawn rectangle
                ...
                c.drawText(textLine, mDelegate.getCalendarPadding(), baseline, textPaint);
            }
        }
